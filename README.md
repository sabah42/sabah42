Salut, je suis sabah assas

**Data Analyst | Passionnée par la Data, la BI et l’Analyse décisionnelle**

Bienvenue sur mon GitHub !  
Tu trouveras ici mes projets **personnels**, mes projets réalisés dans le cadre de ma formation **OpenClassrooms**, ainsi que mon **projet de fin d’étude**.  
J’aime transformer les données en **insights clairs et actionnables**.

---

##  Projets – Personnels
  - [Analyse-les-performances-de-l-entreprise](https://github.com/sabah42/Analyse-les-performances-de-l-entreprise)

     → Analyse exploratoire, indicateurs clés, visualisation
  - [Assistant IA de veille Tech / IA](https://github.com/sabah42/Assistant-IA-automatis-de-veille-Tech-IA)

    → Collecte d’informations, automatisation, NLP
  - [Analyse du Marché Immobilier (2006-2023) - PowerBI](https://github.com/sabah42/Analyse-du-March-Immobilier-2006-2023-Power-BI)

     → Analyse temporelle, tendances, dashboards interactifs


##  Projets – OpenClassrooms
  - [Poultry Market Study: hierarchical clustering and PCA](https://github.com/sabah42/Poultry-Market-Study-hierarchical-clustering-and-PCA)

    → Clustering, ACP, interprétation métier
  - [Détectez des faux billets](https://github.com/sabah42/D-tectez-des-faux-billets)

     → Classification, Machine Learning, évaluation des modèles
  - [Analyse des ventes de votre entreprise](https://github.com/sabah42/Analysez-les-ventes-de-votre-entreprise)

     → Analyse descriptive, KPIs, recommandations
  - [Effectuez une prédiction de revenus](https://github.com/sabah42/Effectuez-une-pr-diction-de-revenus)

     → Modélisation prédictive, régression
  - [Réalisez une étude de santé publique](https://github.com/sabah42/R-alisez-une-tude-de-sant-publique-)

     → Analyse de données, visualisation, recommandations stratégiques
  - [Quelles entreprises correspondent aux métiers de la data aujourd'hui](https://github.com/sabah42/Quelles-entreprises-correspondent-aux-m-tiers-de-la-data-aujourd-hui-)

     → Web scraping, analyse du marché de l’emploi, data analysis



## Projet de fin d’étude
  - [Modélisation de la Volatilité du Taux de Change du Dinar Algérien](https://github.com/sabah42/-Mod-lisation-de-la-Volatilit-du-Taux-de-Change-du-Dinar-Alg-rien)
  
   → Analyse de séries temporelles
  
   → Modélisation statistique et économétrique
   
   → Interprétation économique
   
   → Aide à la décision

*Projet académique avancé démontrant mes compétences analytiques et quantitatives*

---

## Compétences

### Langages
- Python  
- SQL  

### Data Analysis & Data Science
- Collecte, nettoyage et préparation des données
- Analyse exploratoire (EDA)
- Visualisation de données
- Modélisation prédictive
- Classification & clustering
- Analyse de séries temporelles
- ACP (PCA)
- Évaluation et interprétation des modèles

### Business Intelligence
- Power BI
- Création de dashboards
- KPIs & reporting
- Storytelling data

### Outils & Environnements
- Jupyter Notebook
- Git / GitHub
- Excel

### IA & Automatisation
- Notions de NLP
- Veille technologique automatisée
- Bases de l’IA appliquée

--- 
#### Me contacter: 
- **LinkedIn :** [Sabah ASSAS](https://www.linkedin.com/in/sabah-assas-b70413202/)  
- **Email :** assas.sabah@gmail.com  

<!---
sabah42/sabah42 is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
